                ...
                <section class="bg-white py-10">
                    <header class="m-6 space-y-6">
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            Update Pengadaan untuk menyimpan data pengadaan agar selalu konsisten ✨
                        </p>
...
const props = defineProps({
    procurement: {
        type: Object,
        default: () => ({}), // Provide a default empty object to avoid errors
    },
});

const form = useForm({
    tanggal: props.procurement?.tanggal ?? "",
    title: props.procurement?.title ?? "",
    bulan: props.procurement?.bulan ?? "",
    tahun: props.procurement?.tahun ?? "",
    division: props.procurement?.division ?? "",
    status: props.procurement?.status ?? "",
    procurement_number: props.procurement?.procurement_number ?? "",
    remarks: props.procurement?.remarks ?? "",
    _method: "put",
});
...
const submitForm = () => {
    form.put(route("procurement.update", props.procurement.id), {
        onSuccess: () => {
            form.reset();
        },
    });
